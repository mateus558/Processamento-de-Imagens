...
def mean_square_error(img1, img2, format):
    ...
    if width == width2 and height == height2:
        if format == 'TC':
            return squared_error(img1, img2, format) / (width * height)
        elif format == 'RGB':
            return squared_error(img1, img2, format) / (width * height * 3)
    else:
        return 0


# signal_to_noise_ration('lena.jpg', 'lena2.jpg', 'RBG')
def signal_to_noise_ration(img, noisy_img, format):
